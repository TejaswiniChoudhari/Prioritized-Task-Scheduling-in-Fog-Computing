...
public class UserBase extends CloudSim implements GeoLocatable {
	...
	int requestsSent = 0;
	static List<InternetCloudlet> priorityReqs = new ArrayList<InternetCloudlet>(), requests= new ArrayList<InternetCloudlet>();
	static int count =0;
	/** 
	 ...
	 */
	//private Map<Integer, Double> currentRequests;   T
	public Map<Integer, Double> currentRequests;
	/** Records the resonse time statistics grouped by the hour of the day. */
	private HourlyStat hourlyResponseTimeStat;
	
	PriorityRequestScheduling prioritySch = new PriorityRequestScheduling();
		...
		while (Sim_system.running() && !cancelled){
			...
			for (int i = 0; i < requestGroups; i++){
				cloudlet = new InternetCloudlet(get_id() * 100000 + ++id, //Id need not be unique, just used for debugging
												 ...
												 userGroupingFactor);
				requests.add(cloudlet);
				System.out.println("in userbase" + count++);
				//send(Constants.INTERNET, 0.0, Constants.REQUEST_INTERNET_CLOUDLET_TAG, cloudlet); //the sending of the cloudlet is done here............
				messagesSent++;
			...
			int countp=0;
			priorityReqs = prioritySch.setPriorityQ();
			for(int j = 0; j < priorityReqs.size(); j++){
				System.out.println("In priority................:"+ countp++); ///this is not getting called!!
				send(Constants.INTERNET, 0.0, Constants.REQUEST_INTERNET_CLOUDLET_TAG, priorityReqs.get(j)); //it is not processing all the requests!! why? 
				//the user base is getting called fully. but the data center is not getting called for all the requests
			}
			...
			//I need to submit the 3 queues here in currentRequests. The system can be compared by commenting this one line later on. 
	...
	public static List<InternetCloudlet> getReqs(){
		return requests;
	}
